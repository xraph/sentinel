...
func (s *Store) Migrate(ctx context.Context) error {
	...
	for _, f := range files {
		...
		if err != nil {
			return fmt.Errorf("sentinel: %w: read %s: %w", sentinel.ErrMigrationFailed, f.Name(), err)
		}
		if _, err := s.db.ExecContext(ctx, string(data)); err != nil {
			return fmt.Errorf("sentinel: %w: exec %s: %w", sentinel.ErrMigrationFailed, f.Name(), err)
		}
	}
	return nil
}
...
func (s *Store) DeleteSuite(ctx context.Context, suiteID id.SuiteID) error {
	_, err := s.db.NewDelete().Model((*suiteModel)(nil)).Where("id = ?", suiteID.String()).Exec(ctx)
	if err != nil {
...
func (s *Store) DeleteCase(ctx context.Context, caseID id.CaseID) error {
	_, err := s.db.NewDelete().Model((*caseModel)(nil)).Where("id = ?", caseID.String()).Exec(ctx)
	if err != nil {
...
func (s *Store) CountCases(ctx context.Context, suiteID id.SuiteID) (int64, error) {
	count, err := s.db.NewSelect().Model((*caseModel)(nil)).
		Where("suite_id = ?", suiteID.String()).
		Count(ctx)
	if err != nil {
...
func (s *Store) SaveBaseline(ctx context.Context, b *baseline.Baseline) error {
	...
	if b.IsCurrent {
		_, err := s.db.NewUpdate().Model((*baselineModel)(nil)).
...
func (s *Store) DeleteBaseline(ctx context.Context, baselineID id.BaselineID) error {
	_, err := s.db.NewDelete().Model((*baselineModel)(nil)).Where("id = ?", baselineID.String()).Exec(ctx)
	if err != nil {
...
func (s *Store) SetCurrentPromptVersion(ctx context.Context, suiteID id.SuiteID, pvID id.PromptVersionID) error {
	_, err := s.db.NewUpdate().Model((*promptVersionModel)(nil)).
		Set("is_current = ?", false).
		Where("suite_id = ?", suiteID.String()).
		Exec(ctx)
	if err != nil {
